...
class OllamaLauncher(QtWidgets.QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        look_for_llm_action = view_menu.addAction('Look for LLM')
        look_for_llm_action.triggered.connect(self.open_ollama_library)
        ...
        test_ollama_action = help_menu.addAction('Test Ollama (Debug)')
        test_ollama_action.triggered.connect(self.test_ollama_debug)
        help_menu.addSeparator()
    ...
    def apply_theme(self):
        if self.dark_mode:
            # Dark mode styling with bold text, same size as light mode
            dark_style = """
            QMainWindow {
    ...
    def get_installed_llms(self):
        ...
        try:
            # Enhanced cross-platform subprocess settings
            cmd = ["ollama", "list"]
            
            # On Linux, ensure we have proper environment
            env = None
            if platform.system().lower() == 'linux':
                import os
                env = os.environ.copy()
                # Ensure PATH includes common ollama installation locations
                path_additions = [
                    "/usr/local/bin",
                    "/usr/bin", 
                    "/opt/ollama/bin",
            ...
            result = subprocess.run(
                ...
                errors='ignore',
                timeout=30,
                env=env  # Use enhanced environment on Linux
            )
            ...
            if len(lines) < 2:
                print(f"Not enough lines in output: {len(lines)}")
                return []
            
            # More robust parsing - handle different output formats
            llms = []
            for i, line in enumerate(lines[1:], 1):  # Skip header line
                ...
                # Split and get first non-empty part (model name)
                parts = line.split()
                if parts:
                    model_name = parts[0]
                    # Filter out obvious non-model entries
                    if model_name and not model_name.startswith('#') and ':' in model_name:
                        llms.append(model_name)
                        print(f"Found model: {model_name}")
            
            print(f"Total models found: {len(llms)}")
            return llms
            ...
            QtWidgets.QMessageBox.warning(
                ...
                f"Ollama command not found on {platform.system()}.\n\n"
                "Please ensure Ollama is installed and in your PATH.\n"
                "Try running 'ollama --version' in your terminal to verify."
            )
            return []
        except subprocess.CalledProcessError as e:
            QtWidgets.QMessageBox.warning(
                self, "Command Error", 
                f"Ollama list command failed on {platform.system()}:\n\n"
            ...
            QtWidgets.QMessageBox.warning(
                self, "Error", 
                f"Failed to get LLM list on {platform.system()}:\n{e}\n\n"
                f"Error type: {type(e).__name__}"
            )
            return []

    def test_ollama_debug(self):
        """Debug method to test Ollama installation and detection"""
        debug_info = []
        debug_info.append(f"Platform: {platform.system()}")
        debug_info.append(f"Platform Release: {platform.release()}")
        ...
        try:
            ...
            debug_info.append(f"\nOllama Version Command:")
            debug_info.append(f"Return code: {result.returncode}")
            debug_info.append(f"Stdout: {repr(result.stdout)}")
            debug_info.append(f"Stderr: {repr(result.stderr)}")
            
        except FileNotFoundError:
            debug_info.append(f"\nOllama --version: Command not found")
        except Exception as e:
            debug_info.append(f"\nOllama --version error: {e}")
        ...
        try:
            ...
            if platform.system().lower() == 'linux':
                import os
                env = os.environ.copy()
                path_additions = ["/usr/local/bin", "/usr/bin", "/opt/ollama/bin", os.path.expanduser("~/.local/bin")]
                current_path = env.get('PATH', '')
                for path_add in path_additions:
                    if path_add not in current_path:
                        current_path = f"{path_add}:{current_path}"
            ...
            debug_info.append(f"\nOllama List Command:")
            debug_info.append(f"Return code: {result.returncode}")
            debug_info.append(f"Raw stdout: {repr(result.stdout)}")
            debug_info.append(f"Raw stderr: {repr(result.stderr)}")
            ...
            if result.returncode == 0:
                ...
                for i, line in enumerate(lines):
                    debug_info.append(f"  Line {i}: {repr(line)}")
                        ...
                        if line:
                            ...
                            if parts:
                                ...
                                if model_name and ':' in model_name:
                                    debug_info.append(f"  Model {i}: {model_name}")
        ...
        msg.setWindowTitle("Ollama Debug Information")
        msg.setText("Debug information for Ollama detection:")
        msg.setDetailedText("\n".join(debug_info))
        msg.setStandardButtons(QtWidgets.QMessageBox.Ok)
        msg.exec_()

    def check_ollama_startup(self):
        """Check if Ollama is properly installed on startup"""
        try:
            ...
            if result.returncode == 0:
                print("Ollama detected successfully")
                return True
            else:
                print(f"Ollama version check failed with return code: {result.returncode}")
        except FileNotFoundError:
            # Ollama not found in PATH
            if platform.system().lower() == 'linux':
                # Show Linux-specific help
                QtWidgets.QMessageBox.warning(
                    self, "Ollama Not Found - Linux", 
                    "Ollama was not found in your PATH.\n\n"
                    "On Linux, Ollama might be installed in:\n"
                    ...
                    "1. which ollama\n"
                    "2. ollama --version\n\n"
                    "If not installed, visit: https://ollama.ai/install"
                )
            else:
                # Show general help
                self.show_install_ollama_dialog()
    ...
    def validate_ollama_and_get_llms(self):
        """Validate Ollama installation and return LLMs with helpful error messages"""
        try:
            ...
            if version_result.returncode != 0:
                QtWidgets.QMessageBox.warning(
                    self, "Ollama Issue", 
                    f"Ollama is installed but not working properly.\n\n"
                    f"Version check failed with return code: {version_result.returncode}\n"
                    f"Error: {version_result.stderr}\n\n"
                    f"Try running 'ollama --version' in your terminal."
                )
                return []
            
            # Now try to get the LLM list
            llms = self.get_installed_llms()
            
            if not llms:
                # Ollama works but no LLMs found - provide helpful guidance
                msg = QtWidgets.QMessageBox(self)
                msg.setIcon(QtWidgets.QMessageBox.Information)
                msg.setWindowTitle("No LLMs Found")
                
                system_info = f"System: {platform.system()} {platform.release()}"
                
                if platform.system().lower() == 'linux':
                    msg.setText("No LLMs found on Linux system.")
                    msg.setInformativeText(
                        f"{system_info}\n\n"
                        "Ollama is working, but no models are installed.\n\n"
                        "Linux troubleshooting steps:\n"
                        "1. Run 'ollama list' in terminal\n"
                        "2. If empty, install a model: 'ollama run llama2'\n"
                        "3. Check ollama service: 'systemctl status ollama'\n"
                        "4. Check permissions in ~/.ollama/\n\n"
                        "Use 'Install New LLM' button to install models."
                    )
                else:
                    msg.setText("No LLMs found.")
                    msg.setInformativeText(
                        f"{system_info}\n\n"
                        "Ollama is working, but no models are installed.\n\n"
                        "Try installing a model using the 'Install New LLM' button\n"
                        "or run 'ollama run llama2' in your terminal."
                    )
            ...
            if platform.system().lower() == 'linux':
                QtWidgets.QMessageBox.critical(
                    ...
                    "1. Run: which ollama\n"
                    "2. Run: echo $PATH\n"
                    "3. Check: ls -la /usr/local/bin/ollama\n\n"
                    "Install from: https://ollama.ai/install"
                )
            else:
                self.show_install_ollama_dialog()
            return []
            ...
            QtWidgets.QMessageBox.critical(
                self, "Ollama Error", 
                f"Error validating Ollama on {platform.system()}:\n\n{e}\n\n"
                f"Error type: {type(e).__name__}\n\n"
                "Use 'Test Ollama (Debug)' button for more information."
            )
            return []
